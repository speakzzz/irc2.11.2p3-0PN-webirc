...
#define SSL_PRIVKEY_PATH "/home/ircd/ssl/server.key"
#define SSL_CERT_PATH    "/home/ircd/ssl/server.crt"
 ...
 * confsplit
 *
 * Allow changing SS and SU values in ircd.conf on restart and optionally
 * on /REHASH. see doc/confsplit.txt for more information.
 *
 * CONFSPLIT
 *
 *  define to enable
 ...
 * CONFSPLIT_REHASH
 *
 *  (only available if CONFSPLIT is defined)
 *
 *  define to allow ircd.conf values to be applied to a live server on
 *  /REHASH and not only at server startup
 *
 */

/* #define CONFSPLIT */
/* #define CONFSPLIT_REHASH */
...
/************************************************************************
 *
 * CVS3 (see doc/CVS3.txt for more details)
 *
 * CVS3
 *
 *  undef to disable (most of) cvs irc2.11.3 fixes
 *
 */
 ...
 * WHOISTLS
 *
 *  undef to disable, or define to the message you want to show in WHOIS
 *  for clients connected via SSL/TLS
 *
 * WHOISTLS_NOTICE
 *
 *  (only available when WHOISTLS is defined)
 *
 ...
 * HELLOVERRIDE
 *
 *  undef to disable, or define to allow 4th field of A-line to override
 *  HELLO_MSG (empty field reverts to using the default defined below)
 *
 * HELLOVERRIDE_SCH_NOTICES
 *
 *  (only available when HELLOVERRIDE is defined)
 *
 *  undef to disable sending notices about the override to &NOTICES
 *
 */
...
#ifdef HELLOVERRIDE
#define HELLOVERRIDE_SCH_NOTICES
#endif
 ...
 *
 * (only available when WHOISTLS is defined)
 * (non-rfc but de-facto standard use of numeric 489)
 *
 * CMZ_LOCAL
 *
 *  undef to disable, or define to enable channel operators set a local
 *  &channel +z and only allow in users connected securely with SSL/TLS
 *
 * CMZ_ISUPPORT
 *
 *  (only available when CMZ_LOCAL is defined)
 *
 *  undef to disable, or define to enable showing secure channel modes
 *  in ISUPPORT
 *
 * CMZ_LOCAL_NOTICE
 *
 *  (only available when CMZ_LOCAL is defined)
 *
 *  undef to disable sending a notice to clients trying to join a +z
 *  local channel without a SSL/TLS connection
 *
 * CMZ_LOCAL_NOTICE_CHANGE
 *
 *  (only available when CMZ_LOCAL is defined)
 *
 *  undef to disable sending a notice to channels when upgrading or
 *  downgrading the flag (server modes -z+Z and -Z+z)
 *
 */
...
#ifdef CMZ_LOCAL
#define CMZ_ISUPPORT
#define CMZ_LOCAL_NOTICE
#define CMZ_LOCAL_NOTICE_CHANGE
#endif
...
/*
 * Channel jupe (original by jv)
 * (IMPORTANT: the JUPE_FILE needs to exist! So make sure to touch filename before using RESV)
 */
#undef CHANJUPE
...
*/
#define LOG_SERVER_CHANNELS
#ifdef LOG_SERVER_CHANNELS
#define LOG_SCH_ERROR   /* &ERRORS */
#define LOG_SCH_NOTICE  /* &NOTICES */
#define LOG_SCH_KILL    /* &KILLS */
#define LOG_SCH_CHAN    /* &CHANNEL */
#define LOG_SCH_NUM     /* &NUMERICS */
#define LOG_SCH_SERVER  /* &SERVERS */
#define LOG_SCH_HASH    /* &HASH */
#define LOG_SCH_LOCAL   /* &LOCAL */
#define LOG_SCH_SERVICE /* &SERVICES */
#define LOG_SCH_DEBUG   /* &DEBUG */
#define LOG_SCH_AUTH    /* &AUTH */
#define LOG_SCH_SAVE    /* &SAVE */
#define LOG_SCH_WALLOP  /* &WALLOPS */
#define LOG_SCH_CLIENT  /* &CLIENTS (if defined to exist) */
#define LOG_SCH_OPER    /* &OPER */
...
** Also CCL_CHANJUPE is available when CHANJUPE is defined
*/
/* #define CLIENTS_CHANNEL_LEVEL (CCL_CONN|CCL_QUIT) */
#define CLIENTS_CHANNEL_LEVEL (CCL_ALL|CCL_CHANJUPE)

/*
 * CHANJUPE is undefined above, defined here to leave patch untouched
 *
 * -- mh 20200118 */
 ...
 */
#define DEFAULT_SPLIT_USERS   20000
#define DEFAULT_SPLIT_SERVERS 45

#ifdef TEST0PN
#undef DEFAULT_SPLIT_USERS
#undef DEFAULT_SPLIT_SERVERS
...
#ifndef CONFSPLIT
/* we dont want this around if CONFSPLIT is not defined */
#undef CONFSPLIT_REHASH
#endif

#ifdef SPOOF
#ifdef FIND_USERHOST_IPHASH
# error Sorry, FIND_USERHOST_IPHASH is required to be undefined when SPOOF is defined.
#endif
...
#ifdef SPOOF
#ifndef WHOISEXTRA
# error Sorry, WHOISEXTRA define is required when WHOISTLS and SPOOF are both defined.
#endif
#endif
#else
/* we dont want this if WHOISTLS is not defined */
#undef WHOISTLS_NOTICE
#endif

#ifdef HELLOVERRIDE
#ifndef HELLO_MSG
# error Sorry, HELLO_MSG define is required when HELLOVERRIDE is defined.
#endif
#else
/* we dont want this if HELLOVERRIDE is not defined */
#undef HELLOVERRIDE_SCH_NOTICES
#endif

#ifdef CMZ_LOCAL
#ifndef WHOISTLS
# error Sorry, WHOISTLS define is required when CMZ_LOCAL is defined.
#endif
#else
/* we dont want these if CMZ_LOCAL is not defined */
#undef CMZ_ISUPPORT
#undef CMZ_LOCAL_NOTICE
#undef CMZ_LOCAL_NOTICE_CHANGE
#endif
