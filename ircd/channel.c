...
#ifdef CMZ_LOCAL
/* check all channel members on chptr for TLS only secure flag.
 *
 * returns false if theres any non-TLS members, otherwise false
 *
 * note: this only works for local channels/clients
 * -- mh 2020-05-14 */
static int channel_issecureonly(aChannel *chptr)
{
	Reg Link **curr;
	Reg Link *tmp;
	...
	{
		if (!IsTLS(tmp->value.cptr))
		{
			/* found a member without TLS connection flag */
			return(0);
		}
	}

	/* no non-TLS members found */
	return(1);
}

/* check if chptr channel is secure clients only mode (+z or +Z) and
 * if so upgrade/downgrade flag if needed depending on the channel
 * having only TLS secured clients (-z+Z) or mixed clients (-Z+z)
 *
 * note: this only works for local channels/clients
 * -- mh 2020-05-24 */
void channel_check_secureonly(aChannel *chptr)
{
	if ((*chptr->chname == '&') && !IsQuiet(chptr) && IsChCMZ(chptr))
	{
		/* channel is either +z or +Z */

		if (channel_issecureonly(chptr))
		{
			/* channel members are all secure clients */

			if (!(chptr->mode.mode & MODE_ISSECUREONLY))
			{
				/* channel is only +z, upgrade it to +Z */
				chptr->mode.mode &= ~MODE_SECUREONLY;
				chptr->mode.mode |= MODE_ISSECUREONLY;
#ifdef CMZ_LOCAL_NOTICE_CHANGE
				sendto_channel_butserv(chptr, &me, ":%s NOTICE %s :There are no insecure clients on the channel (EXPERIMENTAL local channel mode)",
					ME, chptr->chname
				);
#endif
				sendto_channel_butserv(chptr, &me, ":%s MODE %s -z+Z", ME, chptr->chname);
			}
		}
		else if (chptr->mode.mode & MODE_ISSECUREONLY)
		{
			/* some channel members are not secure and channel is marked +Z
			 * (this shouldnt happen on a local channel, but better safe than sorry)
			 * downgrade it to +z */
			chptr->mode.mode &= ~MODE_ISSECUREONLY;
			chptr->mode.mode |= MODE_SECUREONLY;
#ifdef CMZ_LOCAL_NOTICE_CHANGE
			sendto_channel_butserv(chptr, &me, ":%s NOTICE %s :There are insecure clients on the channel (EXPERIMENTAL local channel mode)",
				ME, chptr->chname
			);
#endif
			sendto_channel_butserv(chptr, &me, ":%s MODE %s -Z+z", ME, chptr->chname);
...
void	remove_user_from_channel(aClient *sptr, aChannel *chptr)
{
	...
	{
		/* update channel secure only mode if needed -- mh 2020-05-14 */
		channel_check_secureonly(chptr);
	}
#endif
...
void	channel_modes(aClient *cptr, char *mbuf, char *pbuf, aChannel *chptr)
{
	*mbuf++ = '+';
	...
	if (chptr->mode.mode & MODE_ISSECUREONLY)
		*mbuf++ = 'Z';
	else if (chptr->mode.mode & MODE_SECUREONLY)
		*mbuf++ = 'z';
#endif
			...
			int *penalty, int parc, char *parv[])
{
	...
	static	int	flags[] = {
#ifdef CMZ_LOCAL
				MODE_ISSECUREONLY, 'Z', MODE_SECUREONLY, 'z',
#endif
...
*/
...
#ifdef CMZ_LOCAL
		case 'z':
			if ((*chptr->chname != '&') || IsQuiet(chptr))
			{
				sendto_one(cptr, replies[ERR_UNKNOWNMODE], ME, BadTo(sptr->name), *curr, chptr->chname);
				break;
			}

			if ((whatt == MODE_DEL) && ((new & MODE_SECUREONLY) || (new & MODE_ISSECUREONLY)))
			{
				/* unset +z and +Z */
				new &= ~(MODE_SECUREONLY | MODE_ISSECUREONLY);
			}
			else if ((whatt == MODE_ADD) && !((new & MODE_ISSECUREONLY) || (new & MODE_SECUREONLY)))
			{
				/* set +z */
				new |= MODE_SECUREONLY;
			}
		...
		case 'Z':
			...
			else if ((whatt == MODE_ADD) && !((new & MODE_SECUREONLY) || (new & MODE_ISSECUREONLY))
			        && !IsServer(cptr))
			{
				/* downgrade to and set +z */
				new &= ~MODE_ISSECUREONLY;
				new |= MODE_SECUREONLY;
			}
...
#ifdef CMZ_LOCAL
	if (((chptr->mode.mode & MODE_ISSECUREONLY) || (chptr->mode.mode & MODE_SECUREONLY)) && !IsTLS(sptr))
	{
#ifdef CMZ_LOCAL_NOTICE
		/* at least irssi doesnt fully pass on ERR_SECUREONLYCHAN for us, so sending
		 * a NOTICE for good measure -- mh 2020-05-03
		 *
		 * this may change in the future: https://github.com/irssi/irssi/issues/1193
		 * -- mh 2020-05-13 */
		sendto_one(sptr, ":%s NOTICE %s :Cannot join channel %s (+z). Not a secure (SSL/TLS) connection (EXPERIMENTAL local channel mode)",
			ME, BadTo(sptr->name), chptr->chname
		);
#endif
...
#ifdef CHANJUPE
        {
            if (MyConnect(sptr) && !is_allowed(sptr, ACL_CHANJUPE) && 
                !is_allowed(sptr, ACL_CHANJUPE_JOIN))
            {
                if (chptr) 
                {
                    /* Channel is already existing */
                    if ((chptr->mode.mode & (MODE_JUPED|MODE_QUIET)) == MODE_JUPED) 
                    {
                        /* Channel is juped and not quiet */
                        sendto_one(sptr, replies[ERR_UNAVAILRESOURCE],
                                         ME, BadTo(parv[0]), name);
# if defined(CLIENTS_CHANNEL) && (CLIENTS_CHANNEL_LEVEL & CCL_CHANJUPE)
                        sendto_flag(SCH_CLIENT, "%s %s %s %s JOIN %s (-)",
		        ...
		        // Check to see if its juped
		        aConfItem *aconf = NULL;
			if (aconf = (aConfItem*)find_chanjupe(name, CONF_CHANJUPE|CONF_CHANJUPE_CREATE))
			{
			    ...
			    continue;
                    ...
                    aConfItem *aconf = NULL; 
                    /* A new channel will be created later */
                    if (aconf = (aConfItem*)find_chanjupe(name, CONF_CHANJUPE|CONF_CHANJUPE_CREATE))
                    {
                        sendto_one(sptr, replies[ERR_UNAVAILRESOURCE],
                                         ME, BadTo(parv[0]), name);
