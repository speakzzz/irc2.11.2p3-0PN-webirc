...
#ifdef CHANJUPE
int report_jlines(aClient *sptr, char *to)
    {
        aConfItem *aconf;

        if (!MyConnect(sptr) || !is_allowed(sptr, ACL_CHANJUPE))
            {
               sendto_one(sptr, replies[ERR_NOPRIVILEGES], ME, BadTo(to)); 
               return 0; 
            }

        for (aconf = jconf; aconf; aconf = aconf->next)
            {
	        if ((aconf->status & (CONF_CHANJUPE|CONF_CHANJUPE_CREATE) == 0))
        {
            /* Better safe than sorry */
            continue;
        }
	
                sendto_one(sptr,":%s %d %s %c %s :%s", ME, RPL_STATSQLINE, BadTo(to),
                                ((aconf->status == CONF_CHANJUPE) ? 'J': 'j'), aconf->host, aconf->passwd);
            }
        return 1;
    }
#endif
...
#if defined(CLIENTS_CHANNEL) && (CLIENTS_CHANNEL_LEVEL & CCL_STATS)
	if (sptr->user && stat) {
		sendto_flag(SCH_CLIENT, "%s %s %s %s STATS %c %s",
			sptr->user->uid, sptr->name, sptr->user->username, sptr->user->host, stat, name);
	}
#endif
...
/* That's useless. Why bother? */
	...
	case 'I' : case 'i' : /* I (and i) conf lines */
		...
		break;
#ifdef CHANJUPE
	case 'J': case 'j' : /* J conf lines */
		if (!report_jlines(sptr, parv[0]))
		{
			return 2;
		}
		break;
