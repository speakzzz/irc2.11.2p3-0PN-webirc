...
#ifdef HELLOVERRIDE
char		*hello_msg   = NULL; /* this is used to store the HELLO_MSG A-line override message when set -- mh 2020-05-12 */
#endif
#ifdef TKLINE
...
#if defined(KLINE) || defined(CHANJUPE)
char *escape_strncpy(char *dst, char *src, size_t n);
#endif
...
char	*oline_flags_to_string(long flags)
{
	...
	if (flags & ACL_CHANJUPE)
		*s++ = 'J';
	else if (flags & ACL_CHANJUPE_JOIN)
		*s++ = 'j';
#endif
	if (s == ofsbuf)
		*s++ = '-';
...
#ifdef CHANJUPE
			/* The possibility of channel juping (J) must be specified explicitly - not within A */
			tmp &= ~(ACL_CHANJUPE|ACL_CHANJUPE_JOIN);
#endif
			break;
...
#endif
#ifdef CHANJUPE
		case 'j': tmp |= ACL_CHANJUPE_JOIN; break;
...
#ifdef CHANJUPE
aConfItem   *find_chanjupe(char *name, int status) 
{
    Reg aConfItem   *aconf;
    if (!name) 
    {
...
#endif
	{
		...
		switch (*tmp)
		{
            ...
            case 'J' : /* Channel jupe */
                ...
                break;
            case 'j' : /* Channel creation jupe */
                aconf->status = CONF_CHANJUPE_CREATE;
                break;
#endif
		...
		do
		{
			...
			if ((tmp = getfield(NULL)) == NULL)
				break;
#ifdef HELLOVERRIDE
			/* update or remove the HELLO_MSG override from the 4th A-line field
			   -- mh 2020-05-12 */
			if (aconf->status == CONF_ADMIN)
			{
				if (!(tmp && *tmp))
				{
					/* no override in A-line. */

					if (hello_msg)
					{
						MyFree(hello_msg);
#ifdef HELLOVERRIDE_SCH_NOTICES
						sendto_flag(SCH_NOTICE, "HELLO_MSG override in A-line removed");
#endif
					}
				}
				else
				{
					...
					if (!(hello_msg && !strcmp(tmp, hello_msg)))
					{
						if (!strcmp(tmp, HELLO_MSG))
						{
							/* override same as default message */
						...
						{
							/* update the override unless out if its too long */

							if (strlen(tmp) < HELLOLEN)
							{
								if (hello_msg)
								{
									MyFree(hello_msg);
#ifdef HELLOVERRIDE_SCH_NOTICES
									sendto_flag(SCH_NOTICE, "HELLO_MSG override in A-line updated");
								}
								else
								{
									sendto_flag(SCH_NOTICE, "HELLO_MSG overridden in A-line");
#endif
								}

								DupString(hello_msg, tmp);
							}
#ifdef HELLOVERRIDE_SCH_NOTICES
							else
							{
								sendto_flag(SCH_NOTICE, "HELLO_MSG override in A-line too long (%u. Max %u)",
									strlen(tmp), (HELLOLEN-1)
								);
							}
#endif
						}
...
#ifdef CONFSPLIT_REHASH
			int dosplitcheck = 0; /* we only need split check if values changed and it is a rehash */
#else
			if (opt != 0) /* this should tell us if its a server (re)start or a /REHASH */
			{
				/* its a server (re)start */
#endif
			...
			if (tmp3 && *tmp3)
			{
				...
				if (*s == '\0')
				{
					...
					if (i != iconf.split_minservers)
					{
						...
						{
							/* its a rehash */
							sendto_flag(SCH_NOTICE, "changed value of SPLIT_SERVERS (min:%d def:%d) from %d to %d",
								SPLIT_SERVERS, DEFAULT_SPLIT_SERVERS, iconf.split_minservers, i);
			...
			if (tmp4 && *tmp4)
			{
				...
				if (*s == '\0')
				{
					...
					if (i != iconf.split_minusers)
					{
						...
						{
							/* its a rehash */
							sendto_flag(SCH_NOTICE, "changed value of SPLIT_USERS (min:%d def:%d) from %d to %d",
								SPLIT_USERS, DEFAULT_SPLIT_USERS, iconf.split_minusers, i);
...
#ifdef CHANJUPE
        if (aconf->status & (CONF_CHANJUPE|CONF_CHANJUPE_CREATE)) 
		    {
			    aconf->next = jconf;
			    jconf = aconf;
		    } else
...
findkline:
	...
	for (; tmp; tmp = tmp->next)
	{
		...
		if (tklines && ((tmp->hold < timeofday) || (tmp->hold == 0)))
			continue;
#endif
		if (tmp->status == (
#ifdef TKLINE
...
int	prep_kline(int tkline, aClient *cptr, aClient *sptr, int parc, char **parv)
{
	...
	int	err = 0;
	char	myuser[(USERLEN*2)+1], myhost[(HOSTLEN*2)+1], myreason[(TOPICLEN*2)+1];
...
int    m_resv(aClient *cptr, aClient *sptr, int parc, char *parv[])
    {
        ...
        escape_strncpy(channel, parv[1], sizeof(channel)-1); 
        escape_strncpy(reason, parv[2], sizeof(reason)-1);

        if ((out = open(JUPE_FILE, O_WRONLY|O_APPEND))==-1)
            {
...
# if defined(CLIENTS_CHANNEL) && (CLIENTS_CHANNEL_LEVEL & CCL_CHANJUPE)
            sendto_flag(SCH_CLIENT, "%s added a RESV for %s with reason: %s",
                                     BadTo(parv[0]), parv[1], parv[2]);
# endif
